...
use crate::rustdoc_parse::transform::intralinks::links::{
...
fn rewrite_links(
    ...
    let RewriteReferenceLinksResult {
        doc,
...
impl ItemPath {
    ...
    fn name(&self) -> Option<&str> {
        self.path_end
            .checked_sub(1)
            .and_then(|i| self.path_shared.get(i))
            .map(String::as_str)
    }
    ...
    fn get_module_path(self, path: &ItemPath) -> Option<ItemPath> {
        match self {
            SymbolType::Crate => {
                assert!(
                    path.is_toplevel(),
                    "a crate should always be in a toplevel path"
                );
                None
            }
            SymbolType::Struct
            ...
            | SymbolType::Static => {
                let p = path.clone().parent().unwrap_or_else(|| {
                    panic!(
                        "item {} of type {:?} should have a parent module",
                        path, self
                    )
                });
                Some(p)
...
fn all_ancestor_paths<'a>(symbols: impl Iterator<Item = &'a ItemPath>) -> HashSet<ItemPath> {
    symbols
        .into_iter()
        .flat_map(ItemPath::all_ancestors)
        .collect()
}

fn extract_markdown_intralink_symbols(doc: &Doc) -> HashSet<ItemPath> {
    let item_paths_inline_links = markdown_link_iterator(&doc.content)
        .items()
    ...
    let item_paths_reference_link_def = markdown_reference_link_definition_iterator(&doc.content)
        .items()
        .filter_map(|l| l.link.link_as_item_path());

    item_paths_inline_links
        .chain(item_paths_reference_link_def)
        .collect()
}
...
fn documentation_url(
    ...
    let mut link = match item_path.anchor {
        ItemPathAnchor::Root => {
            let std_crate_name = item_path
                .path_components()
                .next()
                .expect("a root path should not be empty");
        ...
        ItemPathAnchor::Crate => {
            let base_url = config
                .docs_rs_base_url
                .as_ref()
                .map_or("https://docs.rs", String::as_str);
            let version = config
                .docs_rs_version
                .as_ref()
    ...
    let module_path = typ
        .get_module_path(item_path)
        .expect("item should belong to a module");
    ...
    let name = item_path
        .name()
        .unwrap_or_else(|| panic!("failed to get last component of {}", item_path));
...
fn rewrite_reference_links_definitions(
    ...
    let iter = markdown_reference_link_definition_iterator(&doc.content);
...
mod tests {
    ...
    fn test_item_path_parent() {
        assert_eq!(
            item_path("crate::baz::mumble").parent(),
            Some(item_path("crate::baz"))
        );
        assert_eq!(
            item_path("::std::baz::mumble").parent(),
            Some(item_path("::std::baz"))
        );
    ...
    fn test_item_path_join() {
        assert_eq!(
            item_path("crate::foo").join(&"bar"),
            item_path("crate::foo::bar"),
        );
        assert_eq!(
            item_path("::std::foo").join(&"bar"),
            item_path("::std::foo::bar"),
        );
    ...
    fn test_traverse_module_expore_lazily() {
        ...
        let expected: HashSet<ItemPath> = [item_path("crate"), item_path("crate::module")]
            .into_iter()
            .collect();
    ...
    fn test_documentation_url() {
        let config = IntralinksConfig::default();

        let symbols_type: HashMap<ItemPath, SymbolType> = [(item_path("crate"), SymbolType::Crate)]
            .into_iter()
            .collect();
        ...
        assert_eq!(
            link.as_deref(),
            Some("https://docs.rs/foobini/latest/foobini/")
        );
        ...
        assert_eq!(
            link.as_deref(),
            Some("https://docs.rs/foobini/latest/foobini/amodule/")
        );

        let symbols_type: HashMap<ItemPath, SymbolType> = [(item_path("::std"), SymbolType::Crate)]
            .into_iter()
            .collect();
        ...
        assert_eq!(
            link.as_deref(),
            Some("https://doc.rust-lang.org/stable/std/")
        );

        let symbols_type: HashMap<ItemPath, SymbolType> =
            [(item_path("::std::collections::HashMap"), SymbolType::Struct)]
                .into_iter()
                .collect();
        ...
        let symbols_type: HashMap<ItemPath, SymbolType> =
            [(item_path("crate::amod"), SymbolType::Mod)]
                .into_iter()
                .collect();
        ...
        let symbols_type: HashMap<ItemPath, SymbolType> =
            [(item_path("crate::MyStruct"), SymbolType::Struct)]
                .into_iter()
                .collect();
    ...
    fn test_markdown_impl_items() {
        ...
        let symbols_type: HashMap<ItemPath, SymbolType> = [
            ...
            (item_path("crate::Foo"), SymbolType::Struct),
            (
                item_path("crate::Foo::new"),
                SymbolType::ImplItem(ImplSymbolType::Method),
            ),
            (
                item_path("crate::Foo::a_method"),
                SymbolType::ImplItem(ImplSymbolType::Method),
            ),
            (
                item_path("crate::Foo::another_method"),
                SymbolType::ImplItem(ImplSymbolType::Method),
            ),
            (
                item_path("crate::Foo::no_self"),
                SymbolType::ImplItem(ImplSymbolType::Method),
            ),
            (item_path("crate::amod::Bar"), SymbolType::Struct),
            (
                item_path("crate::amod::Bar::beer"),
                SymbolType::ImplItem(ImplSymbolType::Method),
            ),
            (
                item_path("crate::Foo::Baz"),
                SymbolType::ImplItem(ImplSymbolType::Type),
            ),
            (
                item_path("crate::Foo::number"),
                SymbolType::ImplItem(ImplSymbolType::Const),
            ),
            (item_path("crate::FooAlias"), SymbolType::Type),
            (
                item_path("crate::FooAlias::hello"),
                SymbolType::ImplItem(ImplSymbolType::Method),
            ),
            (item_path("crate::MyEnum"), SymbolType::Enum),
            (
                item_path("crate::MyEnum::hey"),
                SymbolType::ImplItem(ImplSymbolType::Method),
            ),
            (item_path("crate::MyUnion"), SymbolType::Union),
            (
                item_path("crate::MyUnion::sup"),
                SymbolType::ImplItem(ImplSymbolType::Method),
            ),
        ]
        .into_iter()
