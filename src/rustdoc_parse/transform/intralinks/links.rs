...
use crate::rustdoc_parse::transform::intralinks::ItemPath;
use crate::rustdoc_parse::utils::{MarkdownItemIterator, Span};
...
impl Display for MarkdownReferenceLinkDefinition {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let MarkdownReferenceLinkDefinition {
            label: key,
            link,
            raw_title,
        } = self;
...
impl MarkdownReferenceLink {
    ...
    fn new_shortcut(text: String) -> MarkdownReferenceLink {
        MarkdownReferenceLink::Shortcut {
            text: UniCase::unicode(text),
        }
    }
...
pub fn markdown_link_iterator(source: &str) -> MarkdownItemIterator<MarkdownLink> {
    use pulldown_cmark::{Event, LinkType, Options, Parser, Tag};

    // We need to define a callback for broken links so that we can see broken links so that we can
    // strip them.
    let mut broken_link_callback = |_| {
        Some((
            CowStr::Borrowed("fake://link"),
            CowStr::Borrowed("fake title"),
        ))
    };
    ...
    let iter = parser
        ...
        .filter_map(move |(event, range)| match event {
            ...
            Event::End(Tag::Link(LinkType::Inline, ..)) => {
                ...
                let link = MarkdownLink::Inline {
                    link: MarkdownInlineLink { text, link },
                };
            ...
            Event::End(Tag::Link(LinkType::Reference | LinkType::ReferenceUnknown, ..)) => {
                ...
                let link = MarkdownLink::Reference {
                    link: MarkdownReferenceLink::new(text, label),
                };
            ...
            Event::End(Tag::Link(LinkType::Shortcut | LinkType::ShortcutUnknown, ..)) => {
                ...
                let link = MarkdownLink::Reference {
                    link: MarkdownReferenceLink::new_shortcut(text),
                };
...
mod tests {
    ...
    fn test_markdown_link_iterator() {
        let source = "A [some text] [another](http://foo.com), [another][one]";

        let mut iter = markdown_link_iterator(source).items_with_spans();
        ...
        assert_eq!(&source.to_string()[start..end], "[some text]");
        ...
        assert_eq!(&source.to_string()[start..end], "[another](http://foo.com)");
        ...
        assert_eq!(&source.to_string()[start..end], "[another][one]");

        assert_eq!(iter.next(), None);

        let source = "[another](http://foo.com)[another][one]";
        let mut iter = markdown_link_iterator(&source).items_with_spans();
        ...
        let source = "A [some [text]], [another [text2] (foo)](http://foo.com/foo(bar)), [another [] one][foo[]bar]";
        let mut iter = markdown_link_iterator(&source).items_with_spans();
        ...
        assert_eq!(&source.to_string()[start..end], "[text]");
        ...
        assert_eq!(&source.to_string()[start..end], "[text2]");
        ...
        assert_eq!(&source.to_string()[start..end], r"[some \]text]");
        ...
        assert_eq!(
            &source.to_string()[start..end],
            r"[another](http://foo.\(com\))"
        );
        ...
        assert_eq!(&source.to_string()[start..end], r"[another\]][one\]]");

        assert_eq!(iter.next(), None);

        let source = "A `this is no link [link](http://foo.com)`";
        let mut iter = markdown_link_iterator(&source).items_with_spans();
        ...
        let source = "A [link with `code`!](http://foo.com)!";
        let mut iter = markdown_link_iterator(&source).items_with_spans();
        ...
        let mut iter = markdown_link_iterator(&doc).items_with_spans();
        ...
        assert_eq!(&doc.to_string()[start..end], "[some text]");
        ...
        assert_eq!(&doc.to_string()[start..end], "[another][one]");
    ...
    fn test_markdown_reference_link_definition_iterator() {
        ...
        let mut iter = markdown_reference_link_definition_iterator(&doc).items_with_spans();
